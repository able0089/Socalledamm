...
# Login retry: if Discord 429s us at startup (common after a crash loop),
# back off exponentially rather than hammering the API and deepening the ban.
LOGIN_RETRY_DELAYS = [15, 30, 60, 120, 300]  # seconds between attempts
...
async def main() -> None:
    ...
    for attempt, backoff in enumerate(LOGIN_RETRY_DELAYS + [None], start=1):
        # Create a fresh session for each attempt so we never reuse a
        # half-closed connector from a previous failed attempt.
        connector    = aiohttp.TCPConnector(limit=5)
        http_session = aiohttp.ClientSession(connector=connector)
        try:
            # Brief startup pause so Render's health check completes first
            # and to space out rapid redeploys hitting Discord login.
            await asyncio.sleep(2)
            log.info("Starting Discord bot… (attempt %d)", attempt)
            await run_bot()
            # run_bot() only returns on clean shutdown — exit normally.
            break
        except discord.HTTPException as exc:
            if exc.status == 429:
                await http_session.close()
                if backoff is None:
                    log.error("Still rate limited after %d attempts — giving up.", attempt)
                    raise
                log.warning(
                    "Discord rate limited at login (attempt %d/%d) — "
                    "waiting %ds before retry…",
                    attempt, len(LOGIN_RETRY_DELAYS) + 1, backoff,
                )
            ...
            # Always close if we're about to exit the loop entirely
